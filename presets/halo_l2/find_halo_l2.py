"""
Find initial conditions for the L2 halo orbit preset in the Earth-Moon CR3BP.
...
L2 vs L1 differences:
  - L2 is on the far side of the Moon (x > d_M), ~8% farther than L1.
  - The Legendre coefficients c_n differ: notably c₃ flips sign
    (c₃ ≈ -2.64 at L2 vs +3.13 at L1).
  - Smaller c₂ (3.35 vs 5.04) → lower eigenvalues, slightly longer period.
  - The Richardson algebra is identical — only c_n values change.
...
print("=" * 65)
print(f"Finding L2 Halo Orbit ICs  (Az = {AZ_KM:,} km)")
print("=" * 65)
...
print(f"  γ₂ (L2-Moon distance / L) = {h['_gamma']:.6f}")
print(f"  Legendre coefficients: c₂={h['_c2']:.4f}, c₃={h['_c3']:.4f}, c₄={h['_c4']:.4f}")
print(f"  Note: c₃ < 0 at L2 (vs c₃ > 0 at L1) — sign flip from L2 formula")
...
print(f"""
  WARNING: These are APPROXIMATE. With Az/(γL) ≈ 0.31 (even larger
  than L1), the 3rd-order truncation error is substantial. The vy₀
  estimate is off by ~65%. Numerical correction is essential.""")

print("── Stage 2: Differential correction (shooting method) ──")
...
print(f"""
┌─────────────────────────────────────────────────────────┐
│  SIMULATOR PRESET: halo_l2 (JWST-class orbit)            │
├─────────────────────────────────────────────────────────┤
│  x0  = {hc['x0']:.10f}  (tkm)                  │
│  y0  = 0                                                │
│  z0  = {hc['z0']:.10f}  (tkm)                  │
